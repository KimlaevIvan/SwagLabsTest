...
public class AppTest {
    ...
    public void testMailRu() {
        try {
            ...
            String regex = "[a-zA-Z0-9]+(_user)";
            Pattern pattern = Pattern.compile(regex);
            Matcher matcher = pattern.matcher(user_names);
            String user_name_login = "";
            if (matcher.find()) {
                user_name_login = matcher.group();
            }
            ...
            String regex2 = "[a-zA-Z0-9]+(_sauce)";
            Pattern pattern2 = Pattern.compile(regex2);
            Matcher matcher2 = pattern2.matcher(users_password);
            String password = "";
            if (matcher2.find()) {
                password = matcher2.group();
            }
            ...
            for (int i = 1; i <= 6; i++) {
                ...
                Matcher matcher3 = pattern3.matcher(priseTextPage);
                if (matcher3.find()) {
                    priseText = matcher3.group();
                }
                prise = Float.parseFloat(priseText);
